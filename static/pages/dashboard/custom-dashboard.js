...
$(document).ready(function() {
    ...
    // statistic start
    $(function() {
        let ctx = document.getElementById('Statistics-chart').getContext("2d");
        let greenline = ctx.createLinearGradient(500, 0, 100, 0);
        greenline.addColorStop(0, '#fd93a8');
        greenline.addColorStop(1, '#FC6180');
        let blueline = ctx.createLinearGradient(500, 0, 100, 0);
        blueline.addColorStop(1, '#56CCF2');
        blueline.addColorStop(0, '#2F80ED');
        let myChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: ["MON", "TUE", "WED", "APR", "MAY", "JUN", "JUL"],
                datasets: [{
                    label: "Energy",
    ...
    let ctxx = document.getElementById("feedback-chart").getContext("2d");
    let config = {
        type: 'doughnut',
        data: {
            datasets: [{
